...
func handlePatientAge(mfer *Mfer, bytes []byte, length uint32) error {
	...
	ageInDays, err := Binary2Uint32(mfer.Control.ByteOrder, bytes[1:3]...)
	if err != nil {
		return err
	}
	mfer.Helper.Patient.AgeInDays = ageInDays

	birthYear, err := Binary2Uint32(mfer.Control.ByteOrder, bytes[3:5]...)
	if err != nil {
		return err
	}
...
func handleTime(mfer *Mfer, bytes []byte, length uint32) error {
	year, err := Binary2Uint16(mfer.Control.ByteOrder, bytes[0:2]...)
	if err != nil {
		return err
	}
	month := bytes[2]
	...
	minute := bytes[5]
	second := bytes[6]
	miliSec, err := Binary2Uint16(mfer.Control.ByteOrder, bytes[7:9]...)
	if err != nil {
		return err
	}

	microSec, err := Binary2Uint16(mfer.Control.ByteOrder, bytes[9:11]...)
	if err != nil {
		return err
	}
